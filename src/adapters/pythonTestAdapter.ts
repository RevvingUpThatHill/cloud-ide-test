...
export class PythonTestAdapter implements TestAdapter {
    ...
    /**
     * Ensure Python package structure by creating __init__.py files
     * This allows imports to work correctly regardless of where pytest runs from
     */
    private ensurePythonPackageStructure(directory: string): void {
        const srcDir = path.join(directory, 'src');
        if (!fs.existsSync(srcDir)) {
            return;
        }

        const dirsToMakePackages = [
            srcDir,
            path.join(srcDir, 'main'),
            path.join(srcDir, 'test')
        ];

        for (const dir of dirsToMakePackages) {
            if (fs.existsSync(dir)) {
                const initFile = path.join(dir, '__init__.py');
                if (!fs.existsSync(initFile)) {
                    try {
                        // Create empty __init__.py file
                        fs.writeFileSync(initFile, '# Auto-generated by Cloud IDE Test extension\n', 'utf8');
                        console.log(`[Python Adapter] Created ${initFile}`);
                    } catch (error) {
                        console.warn(`[Python Adapter] Failed to create ${initFile}:`, error);
                    }
                }
            }
        }
    ...
    async discoverTests(directory: string): Promise<DiscoveredTest[]> {
        ...
        // Ensure Python package structure exists
        this.ensurePythonPackageStructure(directory);
    ...
    async runTests(directory: string): Promise<TestResult> {
        ...
        // Ensure Python package structure exists before running tests
        this.ensurePythonPackageStructure(directory);
        
        try {
            // Try multiple directory structures to find where tests are located
            // Strategy: Always run from root with proper PYTHONPATH, but specify different test locations
            // 1. Tests in src/test/ with PYTHONPATH set (most common in labs)
            // 2. Tests in root test/ directory with PYTHONPATH=src
            // 3. Tests in root test/ directory (flat structure)
            const srcPath = path.join(directory, 'src');
            
            const command = `
                (cd "${directory}" && PYTHONPATH="${directory}:${srcPath}" python3 -m pytest src/test --verbose --junit-xml=test-results.xml) 2>/dev/null || \
                (cd "${directory}" && PYTHONPATH="${directory}:${srcPath}" python3 -m unittest discover -s src/test -p "*test*.py" -v) 2>/dev/null || \
                (cd "${directory}" && PYTHONPATH="${srcPath}" python3 -m pytest test --verbose --junit-xml=test-results.xml) 2>/dev/null || \
                (cd "${directory}" && PYTHONPATH="${srcPath}" python3 -m unittest discover -s test -p "*test*.py" -v) 2>/dev/null || \
    ...
    private determineSuccessfulCommand(stdout: string, stderr: string, directory: string): string {
        ...
        if (output.includes('pytest') || output.includes('test session starts')) {
            // Check if tests are in src/test/ or root test/
            if (output.includes('src/test') || fs.existsSync(path.join(srcPath, 'test'))) {
                return `cd "${directory}" && PYTHONPATH="${directory}:${srcPath}" python3 -m pytest src/test --verbose --junit-xml=test-results.xml`;
            } else {
                return `cd "${directory}" && PYTHONPATH="${srcPath}" python3 -m pytest test --verbose --junit-xml=test-results.xml`;
            }
        }
        ...
        if (output.match(/test_\w+.*\.\.\.\s+(ok|FAIL|ERROR)/)) {
            // Check if tests are in src/test/ or root test/
            if (output.includes('src.test') || output.includes('src/test') || fs.existsSync(path.join(srcPath, 'test'))) {
                return `cd "${directory}" && PYTHONPATH="${directory}:${srcPath}" python3 -m unittest discover -s src/test -p "*test*.py" -v`;
            } else {
                return `cd "${directory}" && PYTHONPATH="${srcPath}" python3 -m unittest discover -s test -p "*test*.py" -v`;
            }
        }
        
        // Fallback - assume src/test structure with full PYTHONPATH
        return `cd "${directory}" && PYTHONPATH="${directory}:${srcPath}" python3 -m pytest src/test --verbose`;
    }
}
